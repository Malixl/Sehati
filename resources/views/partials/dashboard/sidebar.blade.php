        ...
        <ul class="space-y-2 font-medium mt-4">
            ...
            <li>
                <a href="{{ route('dashboard.skrining.index') }}"
                    ...
                    <svg class="flex-shrink-0 w-5 h-5 transition duration-75 {{ request()->routeIs('dashboard.skrining.*') ? 'text-blue-600' : 'text-gray-500 group-hover:text-blue-600' }}"
            ...
            <li>
                <a href="/dashboard/capaian"
                    ...
                    <svg class="flex-shrink-0 w-5 h-5 transition duration-75 {{ request()->is('dashboard/capaian*') ? 'text-blue-600' : 'text-gray-500 group-hover:text-blue-600' }}"
            ...
            @if(Auth::user()->isSuperAdmin() || Auth::user()->isOwner())
                <li>
                    <a href="/dashboard/posyandu"
                        ...
                        <svg class="flex-shrink-0 w-5 h-5 transition duration-75 {{ request()->is('dashboard/posyandu*') ? 'text-blue-600' : 'text-gray-500 group-hover:text-blue-600' }}"
                            aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"
                            stroke="currentColor">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
            ...
            @if(Auth::user()->isSuperAdmin() || Auth::user()->isOwner())
                <li>
                    <a href="/dashboard/periode"
                        ...
                        <svg class="flex-shrink-0 w-5 h-5 transition duration-75 {{ request()->is('dashboard/periode*') ? 'text-blue-600' : 'text-gray-500 group-hover:text-blue-600' }}"
                            aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"
                            stroke="currentColor">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
