...
class ScreeningController extends Controller
{
    ...
    {
        $activePeriod = \App\Models\ScreeningPeriod::currentlyActive()->first();
        if (!$activePeriod) {
    ...
    {
        // Edge-Case Handling: Check active period first
        $activePeriod = \App\Models\ScreeningPeriod::currentlyActive()->first();
        if (!$activePeriod) {
            return redirect('/')->with('error', 'Mohon maaf, periode skrining baru saja ditutup oleh fasilitas kesehatan terkait.');
        }
        ...
        // IP + Device-based rate limiting (anti-spam bot)
        $throttleKey = 'screening|' . $request->ip() . '|' . ($device->id ?? 'unknown');
        if (RateLimiter::tooManyAttempts($throttleKey, 5)) {
            return redirect('/')->with('error', 'Anda telah mencapai batas pengisian. Silakan coba lagi setelah 1 jam.');
        }
        ...
        // Transaction to ensure no orphaned data if engine fails
        $screeningId = DB::transaction(function () use ($request, $ruleEngine, $device, $activePeriod) {
        ...
        return redirect()->route('screening.result', ['id' => $screeningId]);
    }
